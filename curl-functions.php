<?php
/*
 * ToDO! Get airlines - Begin
 */
function get_airline_data ( $url ) {
    $curl = curl_init();
...
function go_page_data( $url ) {
    ...
    foreach ( $tr as $single_tr ) {
        ...
        $airline[] = get_airline_data ( "https://www.iata.org". $link );
    }
...
function retrieve_data_from_iata(){
    ...
    foreach ($main_array as $airline){
        $key = array_keys($airline);
        $key = $key[0];
        $air = $airline[$key]; // airline's details
